...
func TestHealthCheck_buildHealthStatusFromCR_EmptyPacemakerdState(t *testing.T) {
	controller := &HealthCheck{}
	...
	require.NotNil(t, status, "HealthStatus should not be nil")
	require.Equal(t, statusUnknown, status.OverallStatus, "Status should be Unknown when PacemakerdState is empty")
	require.Empty(t, status.Errors, "Should have no errors when PacemakerdState is empty (missing data = unknown)")
	require.Empty(t, status.Warnings, "Should have no warnings when PacemakerdState is empty")
}
