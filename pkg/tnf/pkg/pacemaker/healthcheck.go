...
const (
	...
	PacemakerStatusResourceName = "cluster"
...
func NewHealthCheck(
	...
	if err := v1alpha1.AddToScheme(scheme); err != nil {
		panic(fmt.Errorf("failed to add scheme for informer: %w", err))
	}

	// Create informer for PacemakerStatus
	klog.Infof("Creating PacemakerStatus informer for group %s", v1alpha1.SchemeGroupVersion.String())
	...
	klog.Infof("PacemakerHealthCheck controller created, will wait for informers to sync before starting")
...
func (c *HealthCheck) getPacemakerStatus(ctx context.Context) (*HealthStatus, error) {
	...
	if time.Since(pacemakerStatus.Status.LastUpdated.Time) > statusStalenessThreshold {
		return newUnknownHealthStatus(fmt.Sprintf("Pacemaker status is stale (last updated: %v)", pacemakerStatus.Status.LastUpdated.Time)), nil
	}
...
func (c *HealthCheck) buildHealthStatusFromCR(pacemakerStatus *v1alpha1.PacemakerCluster) *HealthStatus {
	...
	if pacemakerStatus.Status.Summary == nil {
		klog.V(2).Infof("PacemakerStatus.Status.Summary is nil, status unknown")
		return status
	}
...
func (c *HealthCheck) checkNodeStatus(pacemakerStatus *v1alpha1.PacemakerCluster, status *HealthStatus) {
	...
	if len(nodes) == 0 {
		klog.V(2).Infof("PacemakerStatus has no node information")
...
func (c *HealthCheck) checkResourceStatus(pacemakerStatus *v1alpha1.PacemakerCluster, status *HealthStatus) {
	...
	if len(resources) == 0 {
		klog.V(2).Infof("PacemakerStatus has no resource information")
		return
	}
	...
	for _, resource := range resources {
		...
		if resource.Role == roleStarted && resource.ActiveStatus == v1alpha1.ResourceActiveStatusActive && resource.Node != "" {
