...
func (c *PacemakerLifecycleManager) StartJobControllers(ctx context.Context) error {
	...
	if !transitionComplete {
		...
		return c.startJobControllersWithLock(ctx, k8sNodes)
	}
}
...
func (c *PacemakerLifecycleManager) startJobControllersWithLock(ctx context.Context, nodes []*corev1.Node) error {
	...
	return startTnfJobcontrollersFunc(ctx, nodes, c.controllerContext, c.operatorClient, c.kubeClient, c.kubeInformersForNamespaces, c.etcdInformer)
}
...
// This prevents races with update jobs that may be triggered before setup completes.
